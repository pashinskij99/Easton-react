...
import { coborrowerData } from "../../mock";
//import url from 'url';
import 'url-search-params-polyfill';
//const { URLSearchParams } = url;

/**
 * Props for CoBorrowerCard component.
 * @typedef {Object} CoBorrowerCardProps
...
/**
 * @param {CoBorrowerCardProps} props - Component props
 */

const CoBorrowerCard = () => {
  ...
  const items = useMemo(() => {
    if (borrowerStore.all.length > 0 && !isEmpty(borrowerStore.all[3])) {
      const finalItems = cloneDeep(coborrowerData.filter(item => item.label !== 'SSN' && item.label !== 'Debt' && item.label !== 'ZipCode' && item.label !== 'Relation'));
      const keys = Object.keys(coborrowerInfo);
      keys.forEach((key) => {
        const targetItem = finalItems.find(
          (item) => item.label.replace(/\s/g, "").toLowerCase() === key
        );
        if (targetItem) {
          targetItem.value = coborrowerInfo[key] || 0;
        }
      });
      return finalItems;
    }
    const finalMockData = cloneDeep(coborrowerData);
    let sum = 0;
    finalMockData[finalMockData.length - 1].value = sum;
    window.localStorage.setItem('coborrower', JSON.stringify(finalMockData));
...
const handleShowPopup = () => {
  const payload = {
    submission_id: coborrowerStore.submission_id,
    borrtype: "coborrower",
    date: new Date(),
  };
    ...
    <Box minW={{ sm: "100%", md: "420px" }} maxW={{ sm: "100%", md: "420px" }}>
      ...
      >
        ...
        <Box display="flex" alignItems="center" justifyContent="flex-start">
          ...
          <Text color="white">{coborrowerInfo?.relation} - CoBorrower</Text>
        </Box>
        <Box>
          <IconButton
            size="xs"
        ...
        {items.map((item, index) => (
          <Box
              ...
              >
                ...
                {(item.label === "Income" || item.label === "Total_Income" || item.label === "RentMtge")   ? `$${parseFloat(item.value).toLocaleString()}` : item.value}
                </Text>
                </Box>
            ))}
        </SimpleGrid>
