import {
  Icon, Box,Text,Button,  IconButton,  SimpleGrid,  NumberInput,  NumberInputField,  Progress,  Input,  useColorModeValue,
...
const finalcontractItems = cloneDeep(contractData);
  ...
  useEffect(() => {}, 
  [
    borrowerStore,    rateGrid,    contractdate,    firstPaymentDate,  borrowerStore.all[2]?.amounttofinance,
  ]);
  ...
  useEffect(() => {
    const handleStorageChange = (e) => {
      if (e.key === "vehicleInfoTD[3]" && e.newValue) {
        ...
        if (newVehicleInfo && newVehicleInfo[3] && newVehicleInfo[3].dealerValue) {
          contractItems.find((item) => item.label === "Amount to Finance").value= newVehicleInfo
        }
      }
    };
  ...
  const handleValueChange = (label, nextValue, recalculate) => {
    const updatedContractItems = contractItems.map((i) => {
      if (i.label === label) {
        ...
        const newValue = typeof nextValue === "string" ? parseFloat(nextValue) : nextValue;
  ...
  const handleSave = () => {
    ...
    const storedAppData = storedAppDataString ? JSON.parse(storedAppDataString) : {};
    const storedTermsData = storedTermsDataString ? JSON.parse(storedTermsDataString) : {};
  ...
  const recalculateDates = () => {
    const updatedContractDate =
      formatDate(termsItems.find((item) => item.label === "Contract Date")?.value || new Date());
    const updatedTerm = termsItems.find((item) => item.label === "Term")?.value;
    const updatedFinalDueDate = new Date(updatedContractDate);
    updatedFinalDueDate.setMonth(updatedFinalDueDate.getMonth() + parseInt(updatedTerm));
    const updatedAmountToFinance =
      contractItems.find((item) => item.label === "Amount to Finance")?.value || 0;
  ...
  function calculate_payment(amt, rate, term, contractdate, firstpmtdate) {
    ...
    //Monthly Payments
    const prec_pmt = Math.round((((rate_b2/12)+(rate_b2/12)/((1+(rate_b2/12))**(term_b3)-1))*loan_b1)*100)/100;
    return prec_pmt;
};  
  ...
  useEffect(() => {
    ...
    updatedTermsItems.forEach((item) => {
      item.editable = ['Contract Rate', 'Term', 'Contract Date', 'First Pmt Date'].includes(item.label);
    });
    setTermsItems(updatedTermsItems);
  }, [borrowerStore]);
...
useEffect(() => {
  recalculateDates();
}, [  termsItems.find((item) => item.label === "Contract Rate")?.value,  termsItems.find((item) => item.label === "Term")?.value,  termsItems.find((item) => item.label === "Contract Date")?.value,  termsItems.find((item) => item.label === "First Pmt Date")?.value,  contractItems.find((item) => item.label === "Amount to Finance")?.value,]);
...
const renderValue = (item) => {
  if (item.label === "Contract Rate") {
    ...
    return (item.value);
  } else if (item.label === "Contract Date" || item.label === "First Pmt Date" || item.label === "Final Due Date") {
    return item.value && !isNaN(new Date(item.value)) ? formatDate(new Date(item.value)) : "";
  } else {
...
const recalculateStampTax = (updatedContractItems) => {
  var downPaymentCalc = parseFloat(updatedContractItems.find((item) => item.label === "Down Payment")?.value) || 0;
  var rebateCalc = parseFloat(updatedContractItems.find((item) => item.label === "Rebate")?.value) || 0;
  var tradeInAllowanceCalc = parseFloat(updatedContractItems.find((item) => item.label === "Trade In Allowance")?.value) || 0;
  var oldamttofinCalc = parseFloat(updatedContractItems.find((item) => item.label === "Amount to Finance")?.value) || 0;
  let total = 0;
  updatedContractItems.forEach(item => {
    if (item.label !== "Florida Stamp Tax" || item.label !== "Amount to Finance" || item.label !== "Rebate" || item.label !== "Down Payment" || item.label !== "Trade In Allowance") {
        total += parseFloat(item.value) || 0;
    }});
  total=parseFloat(total);
  var subt = (downPaymentCalc+rebateCalc+tradeInAllowanceCalc)+oldamttofinCalc;
  var amtStampTax = Math.round(((total - subt)*.0037)*100)/100 ;
...
const recalculateAmountToFinance = (updatedContractItems) => {
  ...
  const updatedContractItemsWithStampTax = recalculateStampTax(updatedContractItems);

  var downPaymentCalc = parseFloat(updatedContractItemsWithStampTax.find((item) => item.label === "Down Payment")?.value) || 0;
  var rebateCalc = parseFloat(updatedContractItemsWithStampTax.find((item) => item.label === "Rebate")?.value) || 0;
  var tradeInAllowanceCalc = parseFloat(updatedContractItemsWithStampTax.find((item) => item.label === "Trade In Allowance")?.value) || 0;
  var floridaStampTaxCalc = parseFloat(updatedContractItemsWithStampTax.find((item) => item.label === "Florida Stamp Tax")?.value) || 0;

  let total = 0;
  updatedContractItemsWithStampTax.forEach(item => {
    if (item.label !== "Amount to Finance" && item.label !== "Rebate" && item.label !== "Down Payment" && item.label !== "Trade In Allowance" && item.label !== "Florida Stamp Tax") {
        total += parseFloat(item.value) || 0;
    }
  });
...
useEffect(() => {
  const updatedAmountToFinance = contractItems.find((item) => item.label === "Amount to Finance")?.value || 0;
  const updatedContractRate = termsItems.find((item) => item.label === "Contract Rate")?.value || -999;
  const updatedTerm = termsItems.find((item) => item.label === "Term")?.value;
  const updatedContractDate = formatDate(termsItems.find((item) => item.label === "Contract Date")?.value || new Date());
  const updatedFirstPaymentDate = termsItems.find((item) => item.label === "First Pmt Date")?.value || firstPaymentDate;
  const updatedPayment = calculate_payment(updatedAmountToFinance, updatedContractRate, updatedTerm, updatedContractDate, updatedFirstPaymentDate);
  setPayment(updatedPayment);
}, [contractItems, termsItems]);
...
const renderInput = (item) => {
  if (item.label === "Contract Date" || item.label === "First Pmt Date" || item.label === "Final Due Date") {
    return (
      <input
        type="date"
        // value={item.value ? formatDate(new Date(item.value)) : ""}
        value={item.value ? new Date(item.value).toISOString().slice(0, 10) : ""}
        onChange={(event) =>
    ...
    return (
      <input
        ...
        onChange={(event) => handleValueChange(item.label, event.target.value, true)}
        style={{
          fontSize: "11px",
          maxWidth: "80px",
          maxHeight: "15px",
...
//            border="1px solid gray"
          ...
          >
            ...
            {item.editable ? (
              item.editable ? renderInput(item) : item
              ) : (
              <Text
                fontSize="11px"
  ...
  //            border="1px solid gray"
            ...
            >
              ...
              {item.editable ? (
                ...
                onChange={(nextValue) => handleValueChange(item.label, currParse(nextValue), true)}
                padding= "0px"
                fontSize="10px"
                maxWidth="90px"
                size="xs"
                ...
                color={item.color}
                >
                <NumberInputField style={{paddingRight:2, textAlign:"right"}} />
                </NumberInput>
                            ): (
                <Text
                  fontSize="12px"
