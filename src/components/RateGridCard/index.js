...
import {
  Icon,  Box,  Text,  IconButton,  SimpleGrid,  Table,  Thead,  Tbody,  Tr,  Th,  Td,  NumberInput,NumberInputField,  TableContainer, Button,  Progress,  Modal,  ModalOverlay,  ModalContent,  ModalBody,  ModalFooter,  ModalHeader, FormControl,  FormLabel,  Input,
} from "@chakra-ui/react";
import { FaQuestionCircle, FaList, FaClipboardList } from "react-icons/fa";
import { getRateGridDetails, sendRateSheet, getDealers } from "../../store/rateGrid";
...
const RateGridCard = () => {
  ...
  const [emailAddress, setEmailAddress] = useState(borrowerStore.borrower.email);
  ...
  const [score, setScore] = useState();  
    const [adjValues, setAdjValues] = useState(Array(rateGridStore.rateGrid?.length).fill(0.00));

  const [ltvOverride, setLtvOverride] = useState("");
  const [scoreOverride, setScoreOverride] = useState("");
  const [pmtValues, setPmtValues] = useState(Array(rateGridStore.rateGrid?.length).fill(0));
  var dealeremail= "";
  var customemailsubj=""
  ...
  function calculate_payment(amt, rate, term, contractdate, firstpmtdate) {
    ...
    //Monthly Payments
    const prec_pmt = Math.round(((rate_b2a+rate_b2a/(((1+rate_b2a)**term_b3)-1))*loan_b1)*100)/100;
    return prec_pmt;};  

    
    const calculatePmtValues = useCallback((rates) => {
      if (!rates) return;
    ...
    const adjustRates = useCallback((data, adjValues) => {
  ...
  if (
    (finalLtv >= 50 && finalLtv <= 100) &&
    (finalScore >= 500 && finalScore <= 800)
  ) {
    const payload = {
...
const handleOpen = async() => {
  ...
  var customemailsubj="Mobility Loan Rates for "+borrowerStore.all[0]?.borrower.lastname

  var emailAddresses = dealersData?.payload
  .filter((dealer) => dealer.dealerid.toString().endsWith(dealerIdSubstring))
  .map((dealer) => dealer.emailaddr)
  .join(", ");
  // Update emailAddress state
  dealeremail= emailAddresses; 
  setEmailAddress(dealeremail)
  customemailsubj="Mobility Loan Rates for "+borrowerStore.all[0]?.borrower.lastname
            ...
            <Tr>
              ...
              <Tbody>
                {rateGridStore.rateGrid.map((item, index) => {
                              return (
                    <Tr key={index} style={{color: hideCheckboxes[index] ? "lightgray" : "black"}}>
                    ...
                    <Text fontSize="11px" fontWeight="bold" textAlign="center">
                        {item.term}
                      </Text>
                    </Td>
                    <Td maxW="10px" whiteSpace="nowrap">
                    <Text fontSize="11px" fontWeight="bold">  
                    {percentFormat(Number(item.rate)*100+(Number(adjValues[index] ||0)))}
                    </Text>
                    </Td>
                    <Td>
                    <Text fontSize="11px" fontWeight="bold" textAlign="center">  
                    ...
                    style={{ display: 'block', marginBottom: '10px', width: '40px', textAlign: 'right' }}
                  />
                                    </Td>
                                                                        </Tr>
  )})}
